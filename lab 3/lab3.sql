...
SELECT COUSTOMER_NAME, ACCOUNT_NUMBER,  MAX(BALANCE) AS MAX_NUMBER
FROM ACCOUNT NATURAL JOIN (CUSTOMER NATURAL DEPOSITOR)
WHERE BALANCE = (SELECT MAX(BALANCE) FOME ACCOUNT);
